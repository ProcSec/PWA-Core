export default class Listeners {
    ...
    static add(element, name, callback, options = {}) {
        ...
        const record = (this._registry.get(id) || {}).callbacks || []
        ...
        this._registry.set({ listener, callbacks: record })
    ...
    static remove(element, name, options, number) {
        const id = { element, name, options }
        const recordObj = this._registry.get(id) || { listener: null, callbacks: null }
    ...
    static get(element, name, options, number = null) {
        const lookup = this._registry.get({ element, name, options }) || false
